    ...
    arrange() {
        if (Array.isArray(this.props.children)) {
            ...
            for (let i in this.props.children) {
                ...
                columns = [columns,<div className={"form-group "+this.devices()+" mt-"+this.props.spacing}>{this.props.children[i]}</div>];
                
                if ((i+1) % this.size() == 0) {
                    grid = [grid,<div className={"form-row"}>{columns}</div>];    
                    columns = null;
                }
            }
            ...
            return <form><div className={"form-row"}><div className={"form-group "+this.devices()+" mt-"+this.props.spacing}>{this.props.children}</div></div></form>
        }
    }
